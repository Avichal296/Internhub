...
export default function AdminDashboardClient({
  user,
  pendingInternships: initialPendingInternships,
  pendingCompanies: initialPendingCompanies,
  userStats: initialUserStats,
  internshipStats: initialInternshipStats,
  applicationStats: initialApplicationStats,
}: AdminDashboardClientProps) {
  const [pendingInternships, setPendingInternships] = useState<any[]>(initialPendingInternships);
  const [pendingCompanies, setPendingCompanies] = useState<any[]>(initialPendingCompanies);
  const [userStats, setUserStats] = useState<any[]>(initialUserStats);
  const [internshipStats, setInternshipStats] = useState<any[]>(initialInternshipStats);
  const [applicationStats, setApplicationStats] = useState<any[]>(initialApplicationStats);
                      ...
                      <Card key={internship.id}>
                        ...
                        <CardContent>
                          ...
                          <div className="flex gap-2">
                            <Button
                              onClick={() => handleInternshipAction(internship.id, 'approve')}
                              size="sm"
                            ...
                            <Button
                              onClick={() => handleInternshipAction(internship.id, 'reject')}
                              size="sm"
                              variant="destructive"
                              disabled={loading}
                            >
